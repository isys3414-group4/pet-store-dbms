...
--     CONSTRAINT regex_customer_email CHECK ( REGEXP_LIKE(email, '^[\w\-.]+@([\w\-]+\.)+[\w\-]{2,4}$') OR email IS NULL ),
...
--     CONSTRAINT regex_staff_email CHECK ( REGEXP_LIKE(email, '^[\w\-.]+@([\w\-]+\.)+[\w\-]{2,4}$') OR email IS NULL ) TODO: Fix this
);
CREATE TABLE ADMINS
(
    admin_id INT,
